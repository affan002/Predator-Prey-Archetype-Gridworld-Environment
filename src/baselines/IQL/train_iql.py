...
This modified trainer computes a per-episode "cumulative reward" metric defined as:

    cumulative_reward = (1 / mean_diff) * (sum_prey_rewards + sum_predator_rewards)

where mean_diff = |mean_predator_reward - mean_prey_reward| computed from per-agent
episode totals. The function is robust to division-by-zero via an epsilon.
...
def compute_cumulative_reward_from_episode_rewards(
    ...
    - episode_rewards: mapping agent_name -> total reward for that agent in the episode.
    - name_to_type: optional mapping from agent_name to type (e.g., 'prey'/'predator').
      If omitted, a simple name heuristic is used.
    - eps: numerical floor to avoid division by zero.

    Returns 0.0 if there are no prey or no predators present (can't compute mean diff).
    """
...
def train(
    ...
    env = GridWorldEnv(agents=agents, render_mode=None, size=grid_size, perc_num_obstacle=0, seed=seed)
    ...
    for ep in range(1, episodes + 1):
        ...
        # sum prey and predator totals across agents (here just one each)
        ...
        # compute running means and mean difference
        ...
        if ep % print_every == 0 or ep == 1:
            print(
                f"Ep {ep}/{episodes} | eps={eps:.4f} | captures={capture_count} | "
                ...
                f"last_cumulative={cumulative:.3f} | mean_cumulative={mean_cumulative:.3f}"
            )
